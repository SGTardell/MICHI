...
class MichiApp {
  ...
  initBrainDumpVoiceAndClipboard() {
    ...
    window.addEventListener('focus', () => {
      if (this.currentTab === 'ideas' || this.currentTab === 'all' || this.viewMode === 'clipper') {
  ...
  applyViewMode(mode) {
    this.viewMode = mode;
    localStorage.setItem('MICHI_VIEW_MODE', mode);
    if (mode === 'clipper') {
      document.body.classList.add('mode-clipper');
      const container = document.getElementById('mobileWebClipperContainer');
      if (container) container.style.display = 'flex';
      this.renderMobileClipperFeed();
    } else {
      document.body.classList.remove('mode-clipper');
      const container = document.getElementById('mobileWebClipperContainer');
  ...
  bindMobileClipperEvents() {
    const form = document.getElementById('mobileClipperForm');
    ...
    if (btnPaste && urlInput && !btnPaste.dataset.bound) {
      ...
      btnPaste.addEventListener('click', async () => {
        try {
          ...
          this.showToast('Tap & hold inside input field to paste.');
        }
      });
    }

    const searchInput = document.getElementById('clipperSearchInput');
    if (searchInput && !searchInput.dataset.bound) {
      searchInput.dataset.bound = 'true';
      searchInput.addEventListener('input', (e) => {
        this.renderMobileClipperFeed(e.target.value.trim());
      });
    }
  ...
  saveMobileClip() {
    ...
    const tagsInput = document.getElementById('mobileClipTags');
    ...
    if (!rawUrl && !title) {
      this.showToast('Please enter a link or type a note!');
      return;
    }
    ...
    const activePill = document.querySelector('#clipperCategoryPills .clipper-cat-pill.active');
    const category = activePill ? activePill.dataset.cat : 'General';

    const tags = [category, 'Brain Dump'];
    if (rawTags) {
      const matches = rawTags.match(/#(\w+)/g) || rawTags.split(/\s+/);
      matches.forEach(t => {
  ...
  renderMobileClipperFeed(filterQuery = '') {
  ...
  initMobileClipperVoice() {
    const btnVoice = document.getElementById('btnClipperVoice');
    const titleInput = document.getElementById('mobileClipTitle');
    const statusText = document.getElementById('clipperVoiceStatus');

    if (!btnVoice || !titleInput) return;
    ...
    btnVoice.onclick = () => {
      ...
      try {
        ...
        this.showToast('Speech recognition unavailable.');
      }
    };
    ...
    recognition.onresult = (event) => {
      ...
      if (transcript) {
        ...
        titleInput.value = existing ? existing + ' ' + transcript : transcript;
      }
    };
    ...
    recognition.onerror = () => {
      ...
      if (statusText) statusText.textContent = 'Dictate Note';
    };
